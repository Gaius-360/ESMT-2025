// ====== admin_messages.js (version complète corrigée) ======
...
document.addEventListener("DOMContentLoaded", async () => {
  await fetchAdminSession(); // récupérer session admin avant socket
  initSocket();
  initStudentSearch();
  initLevelSelection();
  initNotificationUI();
...
function initStudentSearch() {
  ...
  const runSearch = debounce(async () => {
    ...
    try {
      ...
      studentList.innerHTML = students.map(s => `<li data-id="${s._id}" data-fullname="${escapeHtml(s.fullname||s.email)}" title="${escapeHtml(s.email||'')}" class="name-search">${escapeHtml(s.fullname||s.email)} <small>(${escapeHtml(s.level||"N/A")})</small></li>`).join("");
...
async function fetchMessages(studentId) {
  try {
    ...
    messages.forEach(addMessageToChat);
    chatMessages.scrollTop = chatMessages.scrollHeight;
  } catch(err){ console.error(err); chatMessages.innerHTML = `<div class="message"><div class="bubble">Impossible de charger les messages.</div></div>`; }
}
